...
class AktoerGateway(
    ...
    suspend fun getAktoerId(
        ...
        httpRequest.header("Nav-Consumer-Id", "pleiepenger-dokument")
        httpRequest.header("Nav-Personidenter", fodselsnummer.value)
        httpRequest.header("Nav-Call-Id", correlationId.id)
        ...
        if (!httpResponse.containsKey(fodselsnummer.value)) {
            throw IllegalStateException("Svar fra '$completeUrl' inneholdt ikke data om det forsespurte fødselsnummeret.")
        }

        val identResponse =  httpResponse.get(key = fodselsnummer.value)
